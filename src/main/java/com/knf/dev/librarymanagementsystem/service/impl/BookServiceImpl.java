...
public class BookServiceImpl implements BookService {
	...
	public List<Book> findBooksByStatus(String status) {
		try {
			// Convert the string status to the corresponding BookStatus enum
			Book.BookStatus bookStatus = Book.BookStatus.valueOf(status.toUpperCase());

			// Return books with the specified status
			return bookRepository.findByStatus(bookStatus);
		} catch (IllegalArgumentException e) {
			// Handle the case where the status string doesn't match any enum value
			return List.of();  // Return an empty list in case of invalid status
		}
	}
